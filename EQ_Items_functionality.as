...
namespace EQ {
  ...
  funcdef void On_tick_fptr( CBlob@ _this );
  funcdef void On_hit_blob_fptr( CBlob@ _this, Vec2f _world_point, Vec2f _velocity, f32 _damage, CBlob@ _hit_blob, u8 _custom_data );
  funcdef  f32 On_hit_fptr( CBlob@ _this, Vec2f _world_point, Vec2f _velocity, f32 _damage, CBlob@ _hitter_blob, u8 _custom_data );
  funcdef void On_health_change_fptr( CBlob@ _this, f32 _old_health );
  funcdef void On_collision_fptr( CBlob@ _this, CBlob@ _blob, bool _solid, Vec2f _normal, Vec2f _point_1, Vec2f _point_2 );
  funcdef void On_end_collision_fptr( CBlob@ _this, CBlob@ _blob );
  funcdef bool Does_collide_with_blob_fptr( CBlob@ _this, CBlob@ _blob );
  funcdef void On_hit_map_fptr( CBlob@ _this, Vec2f _world_point, Vec2f _velocity, f32 _damage, u8 _custom_data );
  funcdef void On_die_fptr( CBlob@ this );
}//EQ
